...
intents.members = True # Good practice for user apps
...
class HazbinBot(commands.Bot):
    ...
    async def setup_hook(self):
        try:
            # Explicitly sync to make sure Discord sees the command
    ...
    # This will log EVERY time Discord tries to talk to the bot
    async def on_interaction(self, interaction: discord.Interaction):
        log(f"[EVENT] Bot {self.token_id} received an interaction: {interaction.type}")
    ...
    async def hazbin(self, interaction: discord.Interaction, text: str):
        ...
        try:
            ...
            log(f"[SUCCESS] Bot {self.token_id} replied successfully.")
        except Exception as e:
            log(f"[ERROR] Bot {self.token_id} failed to reply: {e}")
...
async def main():
    log("[DEBUG] Starting Hazbin Multi-Bot script...")
    ...
    for i in range(1, 31):
        ...
        if token:
            ...
            # REMOVED: bot.tree.add_command(bot.hazbin) - command is already registered via decorator
            bot_tasks.append(bot.start(token))

    if not bot_tasks:
        log("[ERROR] NO TOKENS FOUND! Please add TOKEN_1, TOKEN_2, etc. in Render's Environment tab.")
    else:
        log(f"[DEBUG] Starting {len(bot_tasks)} bots simultaneously...")
